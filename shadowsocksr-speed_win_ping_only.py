...
#        self.x.reversesort = True
    ...
    def getsort(self):
        self.x.header = False
        self.x.border = False
        return self.x.get_string(fields=["idx"])  # return str(self.x[0,1])


test_option = {}
test_option['ping'] = True
test_option['network'] = True
test_option['speed'] = test_option['youtube'] = False
max_cols = 0
...
def close_ssr():
    subprocess.call(
        'taskkill /f /im ShadowsocksR-dotnet4.0-speedtest.exe', stdout=subprocess.PIPE)
...
if len(argv) > 1:
    ...
    url = input("url:")
    # url="http://104.131.144.201/subscrib.txt"
    headers = {
    ...
    for i in ssr_subscribe_decode:
        if(i):
            ...
            print(ParseSsr.base64_decode(decdata))
