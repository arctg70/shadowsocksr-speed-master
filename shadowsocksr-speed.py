...
class DrawTable(object):
    ...
    def append(self, *args, **kwargs):
        if(kwargs):
            color = colored()
            kwargs['network'] = color.greed(
                kwargs['network']) if kwargs['network'] == "Success" else color.red(kwargs['network'])
            kwargs['network'] = color.greed(
                kwargs['network']) if kwargs['youtube'] != 1 else color.red("timeout")
...
def TestOption(screen):
    Option = 0
    menubar = ["1 - All Test", "2 - Ping",
               "3 - Network", "4 - Speed", "5 - Youtube"]
    test_select = [0, 0, 0, 0]
...
def SelectTable(screen):
    ...
    max_line = term_lines - 1 if term_lines - \
        1 < max_select + 3 else max_select + 2
    # print(max_line)
    # while True:
    #   pass
    ...
    while True:
        try:
            if ss_select < ss_select_x:
                ss_select = ss_select_x
            if ss_select > max_select:
                ss_select = max_select
            ...
            if key in [curses.KEY_UP, ord('w'),
                ...
                if key == ord('r'):
                    ...
                    for x in ssr_config:
                        ...
                        select_table.append(
                            select=x['select'], name=x['remarks'])
                if key in [32, curses.KEY_RIGHT, curses.KEY_LEFT]:
                    ssr_config[ss_select -
                               ss_select_x]['select'] = not ssr_config[ss_select - ss_select_x]['select']
                    select_table = DrawSelectTable()
                    for x in ssr_config:
                        select_table.append(
                            select=x['select'], name=x['remarks'])
...
def connect_ssr(ssr):
    ...
    try:
        ...
        if test_option['ping']:
            ...
            cmd = "ping -c 5 %s |grep 'time=' | awk '{print $%s}' |cut -b 6-" % (
        ...
        if test_option['speed']:
            ...
            result['upload'] = round(
                results_dict['upload'] / 1000.0 / 1000.0, 2)
            result['state'] = "Success"
            result['ip'] = results_dict['client']['ip']
            print("speed_test,ping:%s,download:%s,upload:%s" %
                  (result['ping'], result['download'], result['upload']))
...
ssr_subscribe = urllib.request.urlopen(
    f).read().decode('utf-8')  # 获取ssr订阅链接中数据
